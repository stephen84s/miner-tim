...
SharedPreferences file: `mining_config_secure`. Wallet address encrypted with AES-256-GCM (random 12-byte IV prepended to ciphertext; key stored Base64 in prefs). Legacy ECB-encrypted values are transparently migrated on next save.
