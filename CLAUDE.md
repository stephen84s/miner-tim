...
| **Completed** | **LIVE-01** | **Eight-hour live pool run — 600 accepted, 1 rejected, 0 withheld, 0 errors.** First end-to-end run against a real pool since the migration. The 92-test gate proves emitted ARM64 agrees with the *interpreter*; only this shows a **pool** accepts it (no block was found, so not "what Monero accepts"). **601 verifier opportunities, zero disagreements**, with the verifier provably armed — the per-worker line reports *effective* state. Hashrate median **2226.9 H/s** (n=2817), 0 unplanned disconnects, 12 re-logins with 601 found = 601 responses, so nothing was orphaned. Donation cadence matches the documented 5% (realised 4.17% — the run spans no whole number of 100-min cycles). **Review found the entry overstated in five ways** and its rejection timeline unsupported: three submissions were outstanding at that second, all found while their job was current, and `pool_connection.rs` logs responses with **no identifier**, so no response can be attributed to any submission. Withdrawn; the wrong-hash conclusion now rests on 600/601 accepted and 0 withholds instead. Full log committed as `LIVE8H_RUN.log`, verified internally consistent 2877/2877. Not established: 12-thread behaviour, the TLS path, the `off` switch paths, multi-day stability. |
