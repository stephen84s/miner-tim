...
| **Completed** | **PERF-02** | **#1 measured and reverted — null result.** Giving `emit_cvt_packed_int` explicit destinations lets the f-load convert straight into `f_regs(i)`, dropping 2 FMOVs per lane. **Instruction saving exact and confirmed**: `iter_pre` 111→103, **131,072 fewer per hash**, matching the issue's arithmetic. **Time saving not measurable.** All three criteria — *committed before the first run* — failed: phase-1 effect not positive in every paired comparison (+0.25, **-0.02**, +0.07 pp); branch's phase-1 range overlaps `main`'s (6.12 vs 6.15); and at 11 threads the branch sat below `main` (7.19-7.24 vs 7.42-7.71). That was first written up as a weak *regression* signal; **review refuted it from this repo's own data** — BENCH-02's four runs of *unmodified* `main` span +7.11 to +7.50, wider than the 0.25 pp "signal", with +7.11 below the branch's whole range. Withdrawn. Three of six runs failed the pre-registered baseline gate (warm machine), so phase 2 never had enough valid data to decide anything; phase 1's six all passed and are decisive. Review also found the criterion **unpassable as written** — it demanded non-overlapping ranges against a spread the same document put at ~0.4 pp — so the honest verdict is "smaller than this harness can resolve", not "failed three criteria". Not re-run — the criterion forbids retrying until a run cooperates. `compiler.rs` byte-identical to `main`; the change was *correct* (`verify-jit` 92/92 both profiles) and simply bought nothing. Same outcome as `emit_mem_addr`, for the reason the issue gave: latency- and memory-bound, not instruction-count-bound. |
