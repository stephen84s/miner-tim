...
| **Completed** | **LIVE-01** | **Eight-hour live pool run — 600 accepted, 1 rejected, 0 withheld.** First end-to-end run against a real pool since the migration, and the first exercising a day of JIT, harness and release-flow changes. The 92-test gate proves emitted ARM64 agrees with the *interpreter*; only this proves the *network* accepts it. The single rejection was `Invalid job id` — stale, during 20-second job rotation — not a wrong hash: **zero wrong-result rejections in 601 submissions**, and zero verifier withholds across a difficulty range widening 50k→476k. Hashrate median **2226.8 H/s** (10 min avg, n=2817), 0 errors, 0 unplanned disconnects, clean SIGINT shutdown. Donation accounting verified live for the first time: 4 Author + 4 XMRig stints, 2.5 min each per ~100 min = exactly the documented 5% split 50/50, and 8 rotations (full re-logins mid-hash) produced zero rejections. **Recorded gap:** launched with no sleep inhibitor — safe only because the host was on AC (`sleep 0`); on battery (`sleep 1`) an unplug would have truncated it silently. User caught it 34 min in. Full log committed as `LIVE8H_RUN.log`. Not established: 12-thread behaviour, the TLS path, the `off` switch paths, multi-day stability. |
