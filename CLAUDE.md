...
0.  **Process rules, before the task itself.**
    ...
    - **Delegate the mechanical work to Haiku; keep the judgement.** The
      ...
      **The JIT is the exception.** `rust-implementer` is barred by default from
      `src/randomx/jit/`, the emitter and `vm.rs`'s native-loop path, because a
      defect there does not crash: it silently produces wrong hashes, the pool
      rejects the shares, and the user loses money. Sending a cheap model in
      there is the one place where the token saving is a false economy. If that
      code must change, raise the model deliberately, keep the JIT gate in the
      loop, and review with `jit-reviewer`.
