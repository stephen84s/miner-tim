...
| **Completed** | **JIT-01** | **JIT native iteration loop.** Stages A-D done on `feat/jit-native-loop` (MR !1): the 2048-iteration loop is emitted in ARM64 and is now the **default** for rx/0 + full mode + aarch64. Measured **+9.01%** (95% CI +8.70%..+9.32%) at 11 threads via the new paired A/B harness, which also verified ~147k hashes bit-identical against the body JIT. Four rounds of independent review. Also repaired CI, red on `main` since the edition-2024 migration. MR awaiting review/merge. |
