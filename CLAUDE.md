...
0.  **Process rules, before the task itself.**
    ...
    - **Consolidate commits before pushing.** Every push to a PR's head branch
      starts a full pass: five jobs, ~30 runner-minutes, ~15 minutes of
      wall-clock (`jit-macos` is the long pole). Squash or amend locally and
      push once, rather than pushing a series of small fixes. The repository is
      public, so GitHub bills **zero** minutes for this — what is being saved is
      queue time and runner capacity, and the reviewer's time re-reading a head
      that keeps moving. That is still worth saving, but do not let it push you
      into skipping a verification step to avoid a run.
...
| **Completed** | **PROC-05** | **Three CI-hygiene rules recorded in the agent protocol.** User instruction, now bullets in Operational Protocol step 0: (1) Actions run only on branches that have a PR — already the implemented behaviour since CI-03, recorded for its *consequence*, that a push to a branch with no open PR is checked by nothing; (2) rebase on `main` before merging and merge only on green — **stricter than the enforced `strict: true`**, which a merge commit also satisfies, so the rule asks for a rebase to keep the tested tree identical to the landed one; (3) consolidate commits before pushing, since each push to a PR head costs a full ~30 runner-minute / ~15 wall-clock-minute pass. Recorded with the unit corrected: the repo is public, `billable.total_ms` is 0, so what is saved is queue time and reviewer attention, not money. No workflow or code change. |
