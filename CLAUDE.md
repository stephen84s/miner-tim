...
| **Completed** | **BENCH-02** | **Barriered the multi-thread A/B phase (#5).** Each thread ran its own A-B-B-A schedule unsynchronised, so the aggregation's "round i is concurrent across threads" held only while both arms took equal time — assuming the thing being measured. A barrier makes it true. Because a barrier risks trading phase drift for **tail-idle bias** (round time becomes max-over-threads, so fast threads idle), the harness now *measures* that: per-arm across-thread spread, printed every run. It passes — asymmetry small and sign-flipping (-0.40, +0.60 pp). Two paired runs: the barrier halves the aggregate CI (±0.43/±0.64 → ±0.24/±0.29) and nudges the estimate up (+7.27% → +7.40%), the direction the issue predicts, with the headline unchanged and still inside JIT-01's +6.8-7.4%. The issue's second option was implemented *as well*: per-thread paired diffs (n=11, the independent unit) now print alongside the aggregate, since a barrier fixes concurrency but does not make 24 rounds independent. Baselines checked (568-573 H/s single, 4982-5003 at 11) so no run rested on a throttled machine. Bench-only; no `src/` change. |
