...
| **Completed** | **JIT-01** | **JIT native iteration loop.** Stages A-D done on `feat/jit-native-loop` (MR !1): the 2048-iteration loop is emitted in ARM64 and is now the **default** for rx/0 + full mode + aarch64. Measured **+6.8%-+7.4%** at 11 threads across two independent runs (96/96 paired rounds positive; per-run CIs are tighter than the between-run spread and do not describe reproducibility), via the new paired A/B harness, which also verified ~147k hashes bit-identical against the body JIT. Five rounds of independent review (round 5 caught the harness measuring the native loop against itself; the earlier +9.01% claim is retracted). Also repaired CI, red on `main` since the edition-2024 migration. MR awaiting review/merge. |
