...
0.  **Process rules, before the task itself.**
    ...
    - **Reviewer agents.** Independent review uses the repo's own agents in
      ...
      **Delete the reviewer's `REVIEW_*.md` from the branch before merging.**
      ...
      entry and the PR description, then `git rm` it in the final commit. It
      remains readable forever via `git show <sha>:REVIEW_X.md` on the branch.
...
merge; retrieve one with `git show <sha>:REVIEW_X.md`.
