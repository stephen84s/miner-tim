...
binary is **built locally on a Mac** and published with the GitLab CLI (`glab`).

> The CI `release` job does **not** create releases in practice (no macOS runner,
> and it can't attach a binary it can't build). The `glab` flow below is the real
...
- Install and authenticate the GitLab CLI:
  ...
  ```bash
  brew install glab
  glab auth login            # authenticate to gitlab.com
  glab auth status           # should show "Logged in ... as <you>"
  ```
...
5. **Create the release and attach the artifacts** — one command with `glab`:
   ...
   `glab release create` targets the existing tag, creates the GitLab Release, and
   uploads the files as downloadable assets in one step. Verify with:

   ```bash
   glab release view vx.y.z             # confirm the ASSETS section lists both files
   ```
...
runs `make dist` and `glab release create`/`release-cli`. Until then, steps 3–5
