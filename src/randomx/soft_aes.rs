// Software AES round functions using T-tables
// Reference: RandomX src/soft_aes.cpp
//
// Each AES round is computed using 4 T-table lookups XORed together,
// which combines SubBytes, ShiftRows, and MixColumns in a single step.
...
/// Perform a single AES encryption round (SubBytes + ShiftRows + MixColumns + AddRoundKey).
///
/// This matches the RandomX `soft_aesenc` function. The state and key are treated
/// as little-endian 128-bit values (4 x u32 words).
///
/// The C++ implementation reads the 128-bit input as 4 u32 words in reverse order
/// (s0 = word3, s1 = word2, s2 = word1, s3 = word0), applies T-table lookups
/// with ShiftRows byte selection, then writes the output words in the same reversed order.
pub fn soft_aesenc(state: &[u8; 16], key: &[u8; 16]) -> [u8; 16] {
    // Read 4 little-endian u32 words from state
    // s0 = word3 (bytes 12..16), s1 = word2 (bytes 8..12),
    // s2 = word1 (bytes 4..8), s3 = word0 (bytes 0..4)
    let s0 = u32::from_le_bytes([state[12], state[13], state[14], state[15]]);
    let s1 = u32::from_le_bytes([state[8], state[9], state[10], state[11]]);
    let s2 = u32::from_le_bytes([state[4], state[5], state[6], state[7]]);
    let s3 = u32::from_le_bytes([state[0], state[1], state[2], state[3]]);

    // Read key as 4 little-endian u32 words (same reversed order)
    let k0 = u32::from_le_bytes([key[12], key[13], key[14], key[15]]);
    let k1 = u32::from_le_bytes([key[8], key[9], key[10], key[11]]);
    let k2 = u32::from_le_bytes([key[4], key[5], key[6], key[7]]);
    let k3 = u32::from_le_bytes([key[0], key[1], key[2], key[3]]);

    // T-table lookups with ShiftRows byte selection, then XOR with key
    let out0 = TE0[(s0 & 0xff) as usize]
        ^ TE1[((s3 >> 8) & 0xff) as usize]
        ^ TE2[((s2 >> 16) & 0xff) as usize]
        ^ TE3[(s1 >> 24) as usize]
        ^ k0;
    let out1 = TE0[(s1 & 0xff) as usize]
        ^ TE1[((s0 >> 8) & 0xff) as usize]
        ^ TE2[((s3 >> 16) & 0xff) as usize]
        ^ TE3[(s2 >> 24) as usize]
        ^ k1;
    let out2 = TE0[(s2 & 0xff) as usize]
        ^ TE1[((s1 >> 8) & 0xff) as usize]
        ^ TE2[((s0 >> 16) & 0xff) as usize]
        ^ TE3[(s3 >> 24) as usize]
        ^ k2;
    let out3 = TE0[(s3 & 0xff) as usize]
        ^ TE1[((s2 >> 8) & 0xff) as usize]
        ^ TE2[((s1 >> 16) & 0xff) as usize]
        ^ TE3[(s0 >> 24) as usize]
        ^ k3;

    // Write output in the same reversed word order (w=out0 at bytes 12..16, etc.)
    let mut result = [0u8; 16];
    result[12..16].copy_from_slice(&out0.to_le_bytes());
    result[8..12].copy_from_slice(&out1.to_le_bytes());
    result[4..8].copy_from_slice(&out2.to_le_bytes());
    result[0..4].copy_from_slice(&out3.to_le_bytes());
    result
}

/// Perform a single AES decryption round (InvSubBytes + InvShiftRows + InvMixColumns + AddRoundKey).
///
/// This matches the RandomX `soft_aesdec` function. Uses InvShiftRows byte selection
/// pattern which differs from the encryption ShiftRows.
pub fn soft_aesdec(state: &[u8; 16], key: &[u8; 16]) -> [u8; 16] {
    let s0 = u32::from_le_bytes([state[12], state[13], state[14], state[15]]);
    let s1 = u32::from_le_bytes([state[8], state[9], state[10], state[11]]);
    let s2 = u32::from_le_bytes([state[4], state[5], state[6], state[7]]);
    let s3 = u32::from_le_bytes([state[0], state[1], state[2], state[3]]);
    ...
    // T-table lookups with InvShiftRows byte selection, then XOR with key
    let out0 = TD0[(s0 & 0xff) as usize]
        ^ TD1[((s1 >> 8) & 0xff) as usize]
        ^ TD2[((s2 >> 16) & 0xff) as usize]
        ^ TD3[(s3 >> 24) as usize]
        ^ k0;
    let out1 = TD0[(s1 & 0xff) as usize]
        ^ TD1[((s2 >> 8) & 0xff) as usize]
        ^ TD2[((s3 >> 16) & 0xff) as usize]
        ^ TD3[(s0 >> 24) as usize]
        ^ k1;
    let out2 = TD0[(s2 & 0xff) as usize]
        ^ TD1[((s3 >> 8) & 0xff) as usize]
        ^ TD2[((s0 >> 16) & 0xff) as usize]
        ^ TD3[(s1 >> 24) as usize]
        ^ k2;
    let out3 = TD0[(s3 & 0xff) as usize]
        ^ TD1[((s0 >> 8) & 0xff) as usize]
        ^ TD2[((s1 >> 16) & 0xff) as usize]
        ^ TD3[(s2 >> 24) as usize]
        ^ k3;
...
mod tests {
    ...
    #[test]
    fn test_enc_table_sizes() {
        assert_eq!(TE0.len(), 256);
        assert_eq!(TE1.len(), 256);
        assert_eq!(TE2.len(), 256);
        assert_eq!(TE3.len(), 256);
    }

    #[test]
    fn test_dec_table_sizes() {
        assert_eq!(TD0.len(), 256);
        assert_eq!(TD1.len(), 256);
        assert_eq!(TD2.len(), 256);
        assert_eq!(TD3.len(), 256);
    }

    #[test]
    fn test_enc_table_first_last_values() {
        // Verify a few known values from the C++ tables
        assert_eq!(TE0[0], 0xa56363c6);
        assert_eq!(TE0[255], 0x3a16162c);
        assert_eq!(TE1[0], 0x6363c6a5);
        assert_eq!(TE1[255], 0x16162c3a);
        assert_eq!(TE2[0], 0x63c6a563);
        assert_eq!(TE2[255], 0x162c3a16);
        assert_eq!(TE3[0], 0xc6a56363);
        assert_eq!(TE3[255], 0x2c3a1616);
    }

    #[test]
    fn test_dec_table_first_last_values() {
        assert_eq!(TD0[0], 0x50a7f451);
        assert_eq!(TD0[255], 0x4257b8d0);
        assert_eq!(TD1[0], 0xa7f45150);
        assert_eq!(TD1[255], 0x57b8d042);
        assert_eq!(TD2[0], 0xf45150a7);
        assert_eq!(TD2[255], 0xb8d04257);
        assert_eq!(TD3[0], 0x5150a7f4);
        assert_eq!(TD3[255], 0xd04257b8);
    }

    #[test]
    fn test_soft_aesenc_zero_state_zero_key() {
        let state = [0u8; 16];
        let key = [0u8; 16];
        let result = soft_aesenc(&state, &key);
        // With all-zero input, all table lookups use index 0
        // s0=s1=s2=s3=0, so each lookup uses byte 0 from each word
        // out0 = TE0[0] ^ TE1[0] ^ TE2[0] ^ TE3[0]
        let expected_word = TE0[0] ^ TE1[0] ^ TE2[0] ^ TE3[0];
        let wb = expected_word.to_le_bytes();
        // All 4 output words should be the same
        assert_eq!(result[0..4], wb);
        assert_eq!(result[4..8], wb);
        assert_eq!(result[8..12], wb);
        assert_eq!(result[12..16], wb);
    }

    #[test]
    fn test_soft_aesdec_zero_state_zero_key() {
        let state = [0u8; 16];
        let key = [0u8; 16];
        let result = soft_aesdec(&state, &key);
        let expected_word = TD0[0] ^ TD1[0] ^ TD2[0] ^ TD3[0];
        let wb = expected_word.to_le_bytes();
        assert_eq!(result[0..4], wb);
        assert_eq!(result[4..8], wb);
        assert_eq!(result[8..12], wb);
    ...
    #[test]
    fn test_table_rotation_relationship() {
        // The 4 encryption tables are byte-rotations of each other:
        // TE1[i] = TE0[i].rotate_left(8)
        // TE2[i] = TE0[i].rotate_left(16)
        // TE3[i] = TE0[i].rotate_left(24)
        for i in 0..256 {
            assert_eq!(TE1[i], TE0[i].rotate_left(8), "TE1[{i}] rotation mismatch");
            assert_eq!(TE2[i], TE0[i].rotate_left(16), "TE2[{i}] rotation mismatch");
            assert_eq!(TE3[i], TE0[i].rotate_left(24), "TE3[{i}] rotation mismatch");
        }
        // Same for decryption tables
        for i in 0..256 {
            assert_eq!(TD1[i], TD0[i].rotate_left(8), "TD1[{i}] rotation mismatch");
            assert_eq!(TD2[i], TD0[i].rotate_left(16), "TD2[{i}] rotation mismatch");
            assert_eq!(TD3[i], TD0[i].rotate_left(24), "TD3[{i}] rotation mismatch");
        }
    }
}
